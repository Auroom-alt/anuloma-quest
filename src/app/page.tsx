...
export default function Home() {
  ...
  if (screen === 'character') return <CharacterScreen selected={selected} onSelect={(g) => { setSelected(g); setScreen('name'); }} />;
  return (
    <NameScreen
...
function CharacterScreen({ selected, onSelect }: { selected: CharacterGender | null; onSelect: (g: CharacterGender) => void }) {
  return (
    <main style={styles.page}>
      <div style={styles.center}>
...
function NameScreen({ character, heroName, onChange, onStart }: {
  character: CharacterGender; heroName: string; onChange: (v: string) => void; onStart: () => void;
}) {
  return (
    <main style={styles.page}>
      <div style={styles.center}>
        ...
        <input
          type="text" placeholder={character === 'male' ? 'Арья' : 'Лила'}
          value={heroName} onChange={(e) => onChange(e.target.value)}
          maxLength={20} style={styles.input}
        ...
        <button style={{ ...styles.btnGold, opacity: heroName.trim() ? 1 : 0.4 }} onClick={onStart} disabled={!heroName.trim()}>
          Вступить на путь →
        </button>
      </div>
    </main>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '3px' }}>
        ...
        {days.map(day => (
          <div key={day.iso} title={day.iso} style={{
            ...
            border:         day.isToday   ? '2px solid #FBBF24'
                          : day.practiced ? '1px solid rgba(167,139,250,0.3)'
